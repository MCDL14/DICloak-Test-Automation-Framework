...
说明：本文保留 2026-06 Mac 远端验证的历史运行结果，旧记录中的 P0 数量 59 是当时用例集状态，不回写为当前数量。当前本地代码可发现 75 条 P0，其中环境管理 36 条、全局设置 14 条；2026-08-18 新增和调整后的 Local Auth Lab 相关用例已在 Windows APP 上完成涉及范围回归：环境管理侧 6 条通过，全局设置单向同步 2 条在补齐本机忽略配置后通过。macOS“远程执行前同步当前代码”已实现普通运行配置/测试数据同步，以及独立 Local Auth Lab 配置、持久密钥和 SQLite 认证状态同步，但 2026-08-03 实机复验因 `192.168.40.5:22` SSH 超时未发生，最新新增结论尚未在 macOS 实机运行。最新状态见 `README.md`、`本地登录存储模拟站技术方案.md` 与 `任务进度同步文档.md`。
