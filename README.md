...
UI 支持用例发现、按模块筛选、批量选择、实时日志、执行进度、运行结果统计和历史日志查看，并复用 CLI 的恢复、截图、重试、flaky 统计和飞书通知链路。执行页的用例列表优先展示中文业务名称，hover 帮助中保留原始 test id；真正执行时仍把原始 test id 作为 `--case` 参数传给 CLI。用例运行中日志区只展示最近 50 行，避免全量执行时页面被大量日志拖慢；执行进度面板会展示已跑、待跑、运行中和失败/错误用例。执行结束后顶部日志区会自动收敛为失败、错误、异常等未成功日志；完整过程日志仍以运行历史和 `logs/` 文件为准。用例运行中点击 Streamlit 右上角 `Stop` 会同步取消后台执行：本机任务中断独立 CLI 子进程，远程任务向 SSH PTY 发送 `Ctrl+C`。UI 还支持把 Windows 本机和 macOS 远程执行作为一个同步任务并行启动、统一停止，并分别展示两端结果。不要在 UI 任务之外再用 CLI 抢占同一个 APP、CDP 端口、测试账号或业务数据。详细说明见 `UI使用文档.md`。
...
执行页左侧的“显示模块（不影响执行）”和“搜索显示”只控制列表可见性，不会取消用例勾选，也不会改变本机执行范围。搜索支持模块名、中文用例名、测试类、测试方法和完整 test id。每个模块标题行会直接显示已选数量，并可在不展开模块的情况下选中或取消整个模块。
...
会保存全局设置页面配置的用例必须在变更前调用 `GlobalSettingsPage.capture_global_settings_snapshot()` 记录运行前快照，并在 `finally` 中调用 `restore_global_settings_snapshot()` 通过真实 UI 恢复。当前快照覆盖简单复选框、访问网站限制、禁用抓包软件、书签设置开关、环境列表字段权限、环境列表分页、环境列表排序、清除本地缓存，以及数据同步中的 Cookie、Local Storage、IndexedDB、单向同步开关和白名单。新增全局设置用例如果会使用新的配置项，必须先把该配置项加入快照采集、恢复和文档说明，再编写会保存配置的测试流程。

全局设置保存链路会在点击“确定”并等待页面 loading 结束后，优先在 10 秒内检测 Element Plus 顶部 `保存成功` 提示；命中提示时直接认定保存成功，未命中时继续沿用原有切页面重入和配置回显校验。快照恢复和部分高频配置项已加入 no-op 判断：目标状态和当前 UI 状态一致时不再重复点击“确定”、等待保存或重新进入页面，避免长耗时用例在清理阶段做无意义保存。
...
所有全局设置保存链路在点击“确定”后都会先等待页面 loading 消失，再用 10 秒检测顶部 Element Plus 成功提示 `保存成功`。如果提示出现，直接认定本次配置保存成功，跳过保存后的切页面重入校验；如果提示未出现，则继续按原有方式切到其它页面再回到全局设置校验配置回显，原有重试总次数保持不变。

所有会保存全局设置配置的 P0 用例都在变更前记录完整快照，并在 `finally` 中优先清理业务数据、关闭运行中的环境，再恢复全局设置快照。环境管理中会临时开启全局数据同步项的 `test_26` 至 `test_31` 也接入同一快照恢复能力，避免断言失败或中断后永久改变团队级配置。书签上传文件无法仅靠当前页面 UI 反向还原原始文件内容，快照会记录其可见状态并恢复开关；当前自动化基线要求书签设置在用例开始前保持关闭，若后续需要恢复已上传书签文件，应补充可验证的 API 或原始配置恢复路径。
...
- `test_13_global_settings_one_way_sync_disallow_current_account.py`：先记录全局设置快照，再在“数据设置 → 数据同步”确保 `Cookie`、`Local Storage`、`IndexedDB` 已勾选，打开“防止成员覆盖云端数据，导致环境内账号退出登录”开关，等待“白名单”出现后清空并选择 `超管组`，点击“确定”保存；若未捕获 `保存成功` 提示，则重新进入全局设置校验回显。随后返回环境管理，创建默认配置环境 `全局设置-单向同步-禁止当前账号同步`，首次打开后依次访问三个本地模拟站并登录 `MCDL004`、`MCDL005`、`MCDL006`，逐项断言 `已登录`；关闭环境并确认按钮恢复为“打开”后读取基础设置中的 APP 缓存目录，只删除 19 位纯数字直接子目录并确认无残留；再次打开后逐项断言三个站点均为 `未登录`，最后删除环境并校验不存在。用例 `finally` 会尝试关闭环境、删除同名环境，并恢复运行前全局设置快照。2026-08-17 调整收尾逻辑后 Windows 真实单跑通过：`total=1 passed=1 failed=0 errors=0 skipped=0 flaky=0`。
- `test_14_global_settings_one_way_sync_allow_current_account.py`：先记录全局设置快照，再在“数据设置 → 数据同步”确保 `Cookie`、`Local Storage`、`IndexedDB` 已勾选，打开“防止成员覆盖云端数据，导致环境内账号退出登录”开关，等待“白名单”出现后清空并选择 `管理组`，点击“确定”保存；若未捕获 `保存成功` 提示，则重新进入全局设置校验回显。随后返回环境管理，创建默认配置环境 `全局设置-单向同步-允许当前账号同步`，首次打开后依次访问三个本地模拟站并登录 `MCDL004`、`MCDL005`、`MCDL006`，逐项断言 `已登录`；关闭环境并确认按钮恢复为“打开”后读取基础设置中的 APP 缓存目录，只删除 19 位纯数字直接子目录并确认无残留；再次打开后逐项断言三个站点仍为对应账号的 `已登录`，最后删除环境并校验不存在。用例 `finally` 会尝试关闭环境、删除同名环境，并恢复运行前全局设置快照。2026-08-17 Windows 真实单跑通过：`total=1 passed=1 failed=0 errors=0 skipped=0 flaky=0`。
- `test_15_disable_cookie_data_sync.py`：先记录全局设置快照，再在“数据设置 → 数据同步”读取 `Cookie` 勾选状态；若已勾选则取消勾选并保存，若未捕获 `保存成功` 提示，则重新进入全局设置确认 Cookie 仍为未勾选。随后返回环境管理，创建默认配置环境 `自动化-全局设置-不勾选cookie同步`，首次打开后访问 `http://cookie.dicloak.localhost:18080`，使用 `local_auth_lab_login.cookie` 中的 `MCDL004` 账号登录并等待 2 秒，关闭环境且确认按钮恢复为“打开”后断言登录状态为 `已登录`；未删除本地缓存时再次打开并断言仍为 `已登录`；随后进入个人设置基础设置读取 APP 缓存目录，只删除 19 位纯数字直接子目录并确认无残留；第三次打开后断言 Cookie 模拟站为 `未登录`。用例最后删除新建环境，并在 `finally` 中恢复除 Cookie 必须重新勾选外的运行前全局设置快照。
- `test_16_disable_local_storage_data_sync.py`：先记录全局设置快照，再在“数据设置 → 数据同步”读取 `Local Storage` 勾选状态；若已勾选则取消勾选并保存，若未捕获 `保存成功` 提示，则重新进入全局设置确认 Local Storage 仍为未勾选。随后返回环境管理，创建默认配置环境 `自动化-全局设置-不勾选Local Storage同步`，首次打开后访问 `http://localstorage.dicloak.localhost:18080`，使用 `local_auth_lab_login.localstorage` 中的 `MCDL005` 账号登录并等待 2 秒，关闭环境且确认按钮恢复为“打开”后断言登录状态为 `已登录`；未删除本地缓存时再次打开并断言仍为 `MCDL005 / 已登录`；随后进入个人设置基础设置读取 APP 缓存目录，只删除 19 位纯数字直接子目录并确认无残留；第三次打开后断言 Local Storage 模拟站为 `未登录`。用例最后删除新建环境，并在 `finally` 中恢复除 Local Storage 必须重新勾选外的运行前全局设置快照。
