...
框架基础能力已经搭建到可以加载配置、执行环境预检、发现用例、启动 APP、连接 CDP、发送飞书通知和统计执行结果。当前 `tests/p0` 可发现 58 条 P0 用例：环境管理 25 条、全局设置 12 条、环境分组管理 6 条、成员管理 15 条。
...
- `python -c "from streamlit_runner import discover_cases; cases=discover_cases(); print(len(cases))"`：新增用例后发现 58 条 P0 用例。
