...
当前 UI 用例发现应能读取到 81 条 P0 用例：环境管理 39 条、全局设置 17 条、环境分组管理 6 条、成员管理 15 条、代理管理 4 条。环境管理包含三条预置环境数据恢复校验、三条新环境持续保持用例、六条环境单独设置数据同步用例和两条环境单独设置单向同步用例；全局设置包含原 12 条基础设置用例、两条全局设置单向同步用例，以及 Cookie、Local Storage、IndexedDB 三条全局数据同步关闭验证用例。除三条预置数据校验用例外，所有会登录本地模拟站的用例统一读取 `test_data.local_auth_lab_login`。环境单独设置 Cookie、Local Storage、IndexedDB 同步，环境单独设置不勾选 Cookie、Local Storage、IndexedDB 同步，两条环境单独设置单向同步用例、两条全局设置单向同步用例，以及三条全局设置不勾选数据同步用例均已完成 Windows 真实单跑验证。会保存全局设置配置的用例已统一接入运行前快照和 `finally` 恢复，避免 UI 批量执行时因中间断言失败永久改变团队级配置。
...
#### 步骤 4：执行
- 点击 "▶ 运行选中（N 条）" 按钮
- 本机用例由后台线程托管独立 `run.py` 子进程，前台**实时流式**显示日志（固定高度展示，最多保留 200 行）
...
7. Streamlit 右上角 `Stop`、页面重跑或“停止当前任务”请求会设置共享取消事件；Windows 本机先向独立进程组发送 `CTRL_BREAK` 让 `run.py` 执行清理，超时后只按该子进程 PID 强制终止进程树
