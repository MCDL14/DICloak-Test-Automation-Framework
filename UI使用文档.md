...
- "搜索显示" 输入框：按模块、中文用例名、测试类、测试方法或完整 test_id 缩小当前列表显示
...
- 折叠面板内保留每条用例的独立 checkbox，主标签展示中文业务名称，hover 帮助显示原始 test id
...
- 选择节点。
- 确认连接信息并填写本次会话 SSH 密码。
- 选择运行类型。
- 如果运行类型为“执行用例”，确认下方已勾选用例数量。
- 设置执行选项。
- 点击底部运行按钮。

默认页面只展示执行者需要确认的信息。SSH IP、端口和用户名默认使用节点配置或本机缓存；如需修改，可展开“修改 SSH 连接”。检查远程节点、检查远端代码、手动同步当前代码放在“检查和同步”折叠区；命令预览、节点配置、平台能力矩阵放在“高级信息”折叠区。

远程连接信息：

- 页面会显示当前使用的 SSH 用户、主机和端口。
- 展开“修改 SSH 连接”后可填写 SSH IP/主机、端口、用户名。
