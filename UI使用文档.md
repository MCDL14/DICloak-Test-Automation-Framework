...
当前 UI 用例发现应能读取到 75 条 P0 用例：环境管理 36 条、全局设置 14 条、环境分组管理 6 条、成员管理 15 条、代理管理 4 条。环境管理包含三条预置环境数据恢复校验、三条新环境持续保持用例、三条环境单独设置数据同步用例和两条环境单独设置单向同步用例；全局设置包含原 12 条基础设置用例以及两条全局设置单向同步用例。环境单独设置 Cookie、Local Storage、IndexedDB 同步、两条环境单独设置单向同步用例和两条全局设置单向同步用例均已完成 Windows 真实单跑验证。会保存全局设置配置的用例已统一接入运行前快照和 `finally` 恢复，避免 UI 批量执行时因中间断言失败永久改变团队级配置。
...
| 2.25 | 2026-08-17 | 收敛四条单向同步用例测试数据配置，环境单独设置与全局设置的禁止/允许当前账号同步场景统一读取 `environment_one_way_sync`，示例 YAML 不再按用例重复配置 |
