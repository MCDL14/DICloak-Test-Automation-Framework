...
当前 UI 用例发现应能读取到 75 条 P0 用例：环境管理 38 条、全局设置 12 条、环境分组管理 6 条、成员管理 15 条、代理管理 4 条。环境管理包含三条预置环境数据恢复校验、三条新环境持续保持用例、三条环境单独设置数据同步用例、两条环境单独设置单向同步用例，以及两条全局设置单向同步用例；环境单独设置 Cookie、Local Storage、IndexedDB 同步、两条环境单独设置单向同步用例和两条全局设置单向同步用例均已完成 Windows 真实单跑验证。
