...
第一阶段目标不是一次性补齐全部用例，而是先完成一个可运行、可维护、可扩展的自动化框架，并至少跑通 3 条 P0 验证用例。
...
1. CDP 连接后目标页面的识别方式，可能需要通过 URL、标题或页面关键元素综合判断。
2. 批量导入文件上传是否能通过 DOM 的 `input[type=file]` 直接完成。
3. 书签覆盖和追加的数据表字段，需要与 Dicloak 真实模板对齐。
4. 内核删除规则、成员导出校验规则、本地扩展安装校验规则，在具体用例设计时再细化。
...
7. 连接失败时输出明确错误原因。
8. 目标页面识别方式暂未验证，第一版需要在连接后做页面健康检查。
...
python run.py --config config/config.yaml --module environment
python run.py --config config/config.yaml --module test_01_environment_lifecycle.py
python run.py --config config/config.yaml --module p0/test_01_environment_lifecycle.py
python run.py --config config/config.yaml --module tests/p0/test_01_environment_lifecycle.py
python run.py --config config/config.yaml --case test_01_environment_lifecycle
...
├─ tests/
│  ├─ p0/
│  │  ├─ test_01_environment_lifecycle.py
│  │  ├─ test_02_kernel_switch.py
│  │  └─ test_03_batch_import.py
│  └─ p1/
├─ logs/
├─ reports/
├─ screenshots/
...
6. 文件上传方式暂未验证，优先尝试 CDP/Playwright 直接设置文件；如果不可行，再使用 UIAutomation 操作 Windows 文件选择器。
