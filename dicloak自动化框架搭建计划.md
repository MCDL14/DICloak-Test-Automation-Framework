...
2. 当前已完成并验证环境管理模块 25 条 P0 用例。
3. 当前已开始编写并验证全局设置模块 P0 用例 7 条。
4. 当前已开始编写并验证环境分组管理模块 P0 用例 2 条。
5. P0 用例已按业务模块移动到 `tests/p0/` 下的对应模块目录。
6. 已预留 `proxy_management`、`extension_management`、`member_management` 等模块目录。
7. 当前支持按级别、按文件/目录模块、按业务模块、按单条用例运行。
8. 已接入用例前后恢复机制：全局 APP 稳定态恢复不做业务导航，模块级恢复由各模块自行进入模块首页并清理模块状态，当前已实现环境管理和环境分组管理模块恢复入口。
