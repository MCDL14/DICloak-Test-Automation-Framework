...
2. 当前 `tests/p0` 可发现 101 条 P0 用例：环境管理 44 条、全局设置 22 条、扩展管理 8 条、环境分组管理 6 条、成员分组管理 2 条、成员管理 15 条、代理管理 4 条。
...
7. 代理管理已覆盖创建自定义代理、批量创建、创建 NodeMaven 动态代理、批量创建后批量检测 4 条 P0 用例；扩展管理已覆盖本地扩展、市场扩展、谷歌扩展启用、隐藏扩展、编辑所属分组、编辑本地扩展名称、创建/删除扩展分组和编辑扩展分组名称 8 条；成员分组管理已覆盖创建/删除和首行名称编辑/还原 2 条。
8. 环境管理已接入 Cookie、Local Storage、IndexedDB 三条预置环境数据恢复校验、三条新环境持续保持、一条接口创建/查询/删除的新环境 Cookie 持续保持拷贝用例、环境单独设置数据同步/关闭同步/单向同步/清缓存，以及自定义代理和已有代理环境流程；当前共 44 条。
...
6. 支持执行失败重试、失败截图、flaky 统计和模块级熔断设计。
...
2. 内核删除规则、本地扩展安装校验规则，在具体用例设计时再细化。
...
4. 成员导出校验已通过预期 xlsx 文件、导出文件名正则和导出范围比对落地。
5. 成员 open API 用例通过本地环境变量注入 token；接口状态码非 200 时额外重试 3 次，并在异常路径 best-effort 恢复自动化账号状态。
...
配置文件建议使用 YAML。运行配置和测试数据分开维护，例如：
...
5. 本地可保留 `config/config.example.yaml` 和 `config/test_data.example.yaml` 作为模板，真实配置文件加入 `.gitignore`。
6. `config/config.yaml` 只维护运行环境配置，`config/test_data.yaml` 只维护用例测试数据。
...
当前框架已实现通用失败截图能力，由 `core/screenshot.py` 统一封装，并在 `core/result.py` 的 `addFailure` 和 `addError` 中触发。

截图触发时机在用例失败记录阶段，早于用例后恢复流程，便于保留失败现场。默认由 `run.screenshot_on_failure: true` 控制。

截图优先级如下：

1. 如果当前用例持有可用 CDP，优先通过 CDP/Playwright 截取 APP 页面。
2. 如果 CDP 截图失败，回退到桌面截图。
3. 桌面截图优先使用 `mss`，兼容 Windows 和 macOS。
4. Windows 下如果 `mss` 截图失败，再回退到已有 UIAutomation 桌面截图能力。
5. 截图统一保存到 `screenshots/`。
6. 截图成功后返回截图路径，并写入失败摘要和日志；飞书执行总结会通过失败摘要带出截图路径。

截图失败只记录 warning，不覆盖原始用例失败原因，也不阻断后续恢复机制。
...
第一版如果暂时没有 HTML 报告，可以不发送报告链接，只发送总数、通过数、失败数、异常数、跳过数、通过率和失败摘要。后续如果增加报告服务，再扩展 `core/feishu.py` 的执行总结内容。
...
当某条用例执行失败时，APP 可能停留在弹窗、抽屉、下拉框、气泡窗、加载遮罩、编辑页面或某个筛选状态下。恢复机制按三层设计：

1. 全局 APP 稳定态恢复：由 `pages/app_page.py` 负责，只处理通用 UI 阻塞状态，包括关闭 Element Plus 的 message box、dialog、drawer、popover、popper，等待 loading mask 消失，并确认 Dicloak 主页面外壳可操作。
2. 全局恢复不得进入任何业务模块，不允许默认跳转到环境管理，避免影响代理管理、扩展管理、环境分组管理、成员管理、全局设置等后续模块。
3. 模块级恢复：由各模块页面对象提供 `recover_to_module_home()`。当前环境管理模块会进入环境管理列表，并清除环境管理自己的筛选和选中状态；环境分组管理模块会进入环境分组列表，并关闭阻塞浮层。
4. 用例级清理：具体用例创建的数据仍由用例自身清理，例如删除本次创建的环境、标签、代理或成员。
5. 恢复钩子接入在 `core/result.py` 的 unittest result 层，用例开始前和结束后都会尝试恢复；恢复失败会记录 warning，但不覆盖原始测试失败原因。
6. 个别高风险用例可增加场景级兜底恢复。例如四条成员 open API 用例在异常路径会调用 `api_case_recovery.py`，先恢复自动化账号 `status=ENABLED` 和 `disuse_enable=false`，再重新登录自动化账号、切回自动化团队并进入成员列表。

后续新增模块时，需要同步补充对应页面对象的模块级恢复入口。
...
当前第一阶段已完成环境管理模块 44 条 P0 用例，并已完成全局设置 22 条、扩展管理 8 条、环境分组管理 6 条、成员分组管理 2 条、成员管理 15 条和代理管理 4 条 P0 用例，共 101 条。最近一次完整 Windows P0 全量结果仍是 2026-07-01 的 62 条历史快照；当前 101 条不据此宣称全量通过。环境接口独立客户端已支持使用当前 APP 登录态创建环境、保存响应 `data.id`、按环境名称查询 ID 和批量删除环境，并可选传入 APP 当前 `x-device-id`。2026-09-10 已在新拷贝的 Cookie 持续保持用例中首次接入，原 `test_29` 不变；连接已打开 APP 真实单跑 `1/1` 通过，环境、筛选和全局设置均已恢复，原 APP 未关闭。2026-09-22 修复批量编辑环境多分组逐行浮层读取和修改方式重置问题，目标 P0 连续真实运行两次通过；同日为批量导入加入首次点击 idle 后补点及抽屉重开/重新上传兜底，真实 P0 `1/1` 通过。完整 P1 为 237 条。
...
6. `test_06_batch_import_environments.py`：批量导入环境；上传后按确定按钮 loading、抽屉关闭或导入结果判断提交是否启动，首次 idle 时延时复查并补点第二次，两次均无响应才重开抽屉并重新上传。
...
15. `test_15_export_environment.py`：导出环境。
16. `test_16_create_multi_group_environment.py`：创建多分组环境。
17. `test_17_batch_create_multi_group_environments.py`：批量创建多分组环境。
18. `test_18_edit_single_environment_multi_group.py`：单环境编辑多分组。
19. `test_19_batch_edit_environment_multi_group.py`：批量编辑环境多分组。
20. `test_20_create_tag.py`：创建标签。
21. `test_21_create_environment_with_tags.py`：创建带有标签的环境。
22. `test_22_batch_create_environments_with_tags.py`：批量创建带有标签的环境。
...
44. `test_44_new_environment_cookie_persistence_via_api.py`：拷贝新环境 Cookie 持续保持用例，改为接口创建、列表查询确认、APP 搜索后继续三阶段恢复，最后接口删除、列表查询确认并清空 APP 筛选。

全局设置模块：

1. `test_01_disable_view_password.py`：校验禁止查看网站密码。
2. `test_02_disable_browser_devtools.py`：禁止打开浏览器开发者工具。
...
5. `test_05_edit_extension_groups.py`：编辑扩展所属分组并恢复。
6. `test_06_edit_local_extension_name.py`：编辑本地扩展名称并恢复。
7. `test_07_create_extension_group.py`：创建扩展分组，校验名称和备注后删除并校验删除成功。
8. `test_08_edit_extension_group_name.py`：编辑 `扩展分组03` 名称，按备注和创建时间回找断言并还原。

成员分组管理模块：

1. `test_01_create_member_group.py`：创建成员分组，校验列表字段后删除。
2. `test_02_edit_member_group_name.py`：编辑首行成员分组名称并按备注、创建时间回找和还原。

环境分组管理模块：

1. `test_01_create_environment_group.py`：创建环境分组，校验创建成功后删除并校验删除成功。
2. `test_02_group_containing_environment.py`：包含环境的分组，创建分组和归属该分组的环境，删除分组时勾选删除分组下环境，并校验分组和环境都被删除。
...
19. 命令行运行入口和退出码规则。
20. Streamlit 本地 UI 执行入口与接手文档。
21. README 使用说明和进度同步文档。
