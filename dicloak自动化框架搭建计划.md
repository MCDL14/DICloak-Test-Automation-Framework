...
4. P0 用例已按业务模块移动到 `tests/p0/` 下的对应模块目录。
5. 已预留 `proxy_management`、`extension_management`、`environment_group_management`、`member_management`、`global_settings` 等模块目录。
6. 当前支持按级别、按文件/目录模块、按业务模块、按单条用例运行。
7. 已接入用例前后恢复机制：全局 APP 稳定态恢复不做业务导航，模块级恢复由各模块自行进入模块首页并清理模块状态，当前已实现环境管理模块恢复入口。
...
3. 模块级恢复：由各模块页面对象提供 `recover_to_module_home()`。当前环境管理模块会进入环境管理列表，并清除环境管理自己的筛选和选中状态。
...
当前第一阶段已完成环境管理模块 25 条 P0 用例，并已开始补充全局设置模块 P0 用例。
