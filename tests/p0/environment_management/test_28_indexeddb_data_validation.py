...
CASE_MODULE = "环境管理"
ENVIRONMENT_NAME = "IndexedDB数据校验"
EXPECTED_LOGIN_STATUS = "已登录"


class TestIndexedDBDataValidation(unittest.TestCase):
    ...
    def test_indexeddb_data_survives_close_reopen_and_local_cache_deletion(self) -> None:
        ...
        try:
            ...
            self.logger.info("IndexedDB data sync is enabled: changed=%s", indexeddb_sync_changed)
            ...
            first_status = self._open_read_indexeddb_status_and_close(
                environment_page,
            ...
            assert_equal(
                ...
                f"首次打开环境后 IndexedDB 模拟站登录状态错误: actual={first_status}",
            )

            # 该 3 秒是用例明确要求的云端同步等待窗口，不作为页面加载同步手段。
            time.sleep(3)

            second_status = self._open_read_indexeddb_status_and_close(
                environment_page,
                environment_open_timeout=environment_open_timeout,
                environment_close_timeout=environment_close_timeout,
                kernel_process_timeout=kernel_process_timeout,
            ...
            assert_equal(
                second_status,
                EXPECTED_LOGIN_STATUS,
                f"再次打开环境后 IndexedDB 模拟站登录状态错误: actual={second_status}",
            )
            ...
            third_status = self._open_read_indexeddb_status_and_close(
                environment_page,
                environment_open_timeout=environment_open_timeout,
                environment_close_timeout=environment_close_timeout,
                kernel_process_timeout=kernel_process_timeout,
            ...
            assert_equal(
                third_status,
                EXPECTED_LOGIN_STATUS,
                f"删除本地缓存并恢复后 IndexedDB 模拟站登录状态错误: actual={third_status}",
            )
        finally:
            try:
                environment_page.open_list()
