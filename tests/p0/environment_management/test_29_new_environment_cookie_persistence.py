...
class TestNewEnvironmentCookiePersistence(unittest.TestCase):
    ...
    def test_new_environment_cookie_survives_close_reopen_and_local_cache_deletion(self) -> None:
        ...
        credentials = self.config.get("test_data", {}).get("environment_new_cookie_persistence", {})
        assert_true(
            isinstance(credentials, dict),
            "test_data.environment_new_cookie_persistence 必须是映射配置",
        )
        ...
        assert_equal(
            username,
            EXPECTED_ACCOUNT,
            "新环境 Cookie 持续保持用例账号配置错误",
        )
        assert_true(
            bool(password) and not password.startswith("请在"),
            "请在 config/test_data.yaml 中配置新环境 Cookie 持续保持用例的登录密码",
        )
            ...
            with KernelCDPSession(
                ...
                if login_username:
                    local_auth_page.login(
                        login_username,
                        login_password,
                        run_id="new-environment-cookie-persistence",
                    )
