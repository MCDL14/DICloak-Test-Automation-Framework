...
CASE_MODULE = "环境管理"
ENVIRONMENT_NAME = "自动化-新环境IndexedDB持续保持"
EXPECTED_LOGIN_STATUS = "已登录"
EXPECTED_ACCOUNT = "MCDL006"


class TestNewEnvironmentIndexedDBPersistence(unittest.TestCase):
    ...
    def test_new_environment_indexeddb_survives_close_reopen_and_local_cache_deletion(self) -> None:
        ...
        credentials = self.config.get("test_data", {}).get(
            "environment_new_indexeddb_persistence",
            {},
        )
        assert_true(
            isinstance(credentials, dict),
            "test_data.environment_new_indexeddb_persistence 必须是映射配置",
        )
        ...
        assert_equal(
            username,
            EXPECTED_ACCOUNT,
            "新环境 IndexedDB 持续保持用例账号配置错误",
        )
        assert_true(
            bool(password) and not password.startswith("请在"),
            "请在 config/test_data.yaml 中配置新环境 IndexedDB 持续保持用例的登录密码",
        )
        ...
        try:
            ...
            self.logger.info(
                "Global settings page loaded with at least %s checked checkboxes before IndexedDB read",
                global_settings_page.MINIMUM_CHECKED_CHECKBOXES,
            )
            indexeddb_sync_changed = global_settings_page.ensure_indexeddb_data_sync_enabled()
            assert_true(
                global_settings_page.indexeddb_data_sync_enabled(),
                "数据设置 → 数据同步中的 IndexedDB 未保持勾选状态",
            )
            self.logger.info(
                "IndexedDB data sync is enabled: changed=%s",
            ...
            assert_equal(
                ...
                f"新建环境首次登录后的 IndexedDB 模拟站状态错误: actual={first_status}",
            )
            assert_equal(
                first_account,
                EXPECTED_ACCOUNT,
            ...
            assert_equal(
                second_status,
                EXPECTED_LOGIN_STATUS,
                f"再次打开新环境后的 IndexedDB 模拟站状态错误: actual={second_status}",
            )
            assert_equal(
                second_account,
                EXPECTED_ACCOUNT,
            ...
            assert_equal(
                third_status,
                EXPECTED_LOGIN_STATUS,
                f"删除本地缓存并恢复后 IndexedDB 模拟站状态错误: actual={third_status}",
            )
            assert_equal(
                third_account,
                EXPECTED_ACCOUNT,
            ...
            with KernelCDPSession(
                ...
                if login_username:
                    local_auth_page.login(
                        login_username,
                        login_password,
                        run_id="new-environment-indexeddb-persistence",
                    )
