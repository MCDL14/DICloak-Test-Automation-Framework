...
CASE_MODULE = "环境管理"
ENVIRONMENT_NAME = "自动化-环境单独设置-Local Storage同步"
EXPECTED_LOGIN_STATUS = "已登录"
EXPECTED_ACCOUNT = "MCDL005"
EXPECTED_SYNC_ITEMS = ["Local Storage"]
CREDENTIALS_DATA_KEY = "environment_new_local_storage_persistence"


class TestIndividualEnvironmentLocalStorageSync(unittest.TestCase):
    ...
    def test_individual_environment_local_storage_sync_survives_close_reopen_and_local_cache_deletion(self) -> None:
        ...
        assert_equal(
            username,
            EXPECTED_ACCOUNT,
            "新环境 Local Storage 持续保持共享配置账号错误",
        )
        assert_true(
        ...
        try:
            ...
            selected_sync_items = environment_page.create_environment_with_custom_data_sync(
                ENVIRONMENT_NAME,
                EXPECTED_SYNC_ITEMS,
            )
            assert_equal(
            ...
            with KernelCDPSession(
                ...
                if login_username:
                    local_auth_page.login(
                        login_username,
                        login_password,
                        run_id="individual-environment-local-storage-sync",
                    )
