...
CASE_MODULE = "环境管理"
ENVIRONMENT_NAME = "自动化-环境单独设置-单向同步-允许当前账号同步"
...
class TestIndividualEnvironmentOneWaySyncAllowCurrentAccount(unittest.TestCase):
    ...
    def test_one_way_sync_allows_current_account_data_upload(self) -> None:
        ...
        try:
            ...
            self._assert_sites_logged_in(second_statuses, stage="删除本地缓存后再次打开")
                ...
                for site in SITE_DEFINITIONS:
                    ...
                    if credentials_by_site:
                        ...
                        local_auth_page.login(
                            username,
                            password,
                            run_id=f"individual-one-way-sync-allow-{site_id}",
                        )
                    ...
                    self.logger.info(
                        "One-way sync allow login status captured: "
