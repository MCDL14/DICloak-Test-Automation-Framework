...
CASE_MODULE = "环境管理"
ENVIRONMENT_NAME = "自动化-环境单独设置-不勾选IndexedDB同步"
EXPECTED_ACCOUNT = "MCDL006"
DISABLED_SYNC_ITEMS = ["IndexedDB"]
LOGGED_IN_STATUS = "已登录"
LOGGED_OUT_STATUS = "未登录"


class TestIndividualEnvironmentDisableIndexedDBSync(unittest.TestCase):
    ...
    def test_indexeddb_not_restored_after_cache_deletion_when_individual_indexeddb_sync_disabled(
        self,
    ) -> None:
        ...
        try:
            ...
            assert_true(
                "IndexedDB" not in selected_sync_items,
                f"创建环境时 IndexedDB 数据同步未保持取消勾选: actual={selected_sync_items}",
            )
            ...
            with KernelCDPSession(
                ...
                if login_username:
                    local_auth_page.login(
                        login_username,
                        login_password,
                        run_id="individual-environment-indexeddb-sync-disabled",
                    )
