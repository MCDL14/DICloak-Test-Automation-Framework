...
CASE_MODULE = "环境管理"
ENVIRONMENT_NAME = "自动化-环境单独设置-清除本地全部缓存-每次都清除-不同步云端数据"
...
class TestIndividualEnvironmentClearAllCacheEveryOpenNoCloudSync(unittest.TestCase):
    ...
    def test_clear_all_cache_every_open_without_cloud_sync_clears_site_login_state(self) -> None:
        ...
        try:
            ...
            first_statuses = self._open_visit_sites_and_close(
                ...
                run_id_prefix="clear-all-cache-no-cloud-sync-first",
            )
            self._assert_sites_logged_in(first_statuses, stage="首次打开登录后")

            environment_page.search_environment(ENVIRONMENT_NAME)
            environment_page.edit_environment_clear_all_local_cache_every_open_no_cloud_sync_data(ENVIRONMENT_NAME)
            environment_page.search_environment(ENVIRONMENT_NAME)
            assert_true(
            ...
            second_statuses = self._open_visit_sites_and_close(
                ...
                kernel_cdp_probe_timeout=kernel_cdp_probe_timeout,
                http_probe_timeout=http_probe_timeout,
                stage="启用清除本地缓存且不同步云端后再次打开",
            )
            self._assert_sites_logged_out(second_statuses, stage="启用清除本地缓存且不同步云端后再次打开")
        ...
        try:
            ...
            with KernelCDPSession(
                ...
                for site in SITE_DEFINITIONS:
                    ...
                    self.logger.info(
                        "Clear-all-cache no-cloud-sync login status captured: "
