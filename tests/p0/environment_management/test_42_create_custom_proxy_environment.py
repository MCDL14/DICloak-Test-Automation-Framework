...
CASE_MODULE = "环境管理"
ENVIRONMENT_NAME = "自动化-使用-自定义代理-的环境"
...
def chrome_webstore_failure_message(result: KernelPageResult) -> str:
    return (
        "使用自定义代理的环境无法访问 Chrome Web Store: "
    ...
    def test_create_custom_proxy_environment_open_close_delete(self) -> None:
        ...
        try:
            ...
            try:
                ...
                self.logger.info(
                    "Custom proxy Chrome Web Store connectivity: reachable=%s target_url=%s "
                    "title=%r error=%r text_length=%s",
                    reachable,
                ...
                delayed_assertions.append(
                    AssertionError(
                        "使用自定义代理的环境访问 Chrome Web Store 时发生异常: "
