...
CASE_MODULE = "全局设置"
ENVIRONMENT_NAME = "自动化-全局设置-不勾选cookie同步"
EXPECTED_ACCOUNT = "MCDL004"
LOGGED_IN_STATUS = "已登录"
LOGGED_OUT_STATUS = "未登录"


class TestDisableCookieDataSync(unittest.TestCase):
    ...
    def test_cookie_not_restored_after_cache_deletion_when_global_cookie_sync_disabled(self) -> None:
        ...
        username, password = self._cookie_credentials()
        self._ensure_cookie_lab_user(username, password)
        ...
        try:
            ...
            cookie_sync_changed = global_settings_page.ensure_cookie_data_sync_disabled()
            assert_true(
                not global_settings_page.cookie_data_sync_enabled(),
                "数据设置 → 数据同步中的 Cookie 未保持取消勾选状态",
            )
            self.logger.info("Cookie data sync is disabled: changed=%s", cookie_sync_changed)
            ...
            assert_equal(
                first_status,
                LOGGED_IN_STATUS,
                f"首次打开登录后 Cookie 模拟站状态错误: actual={first_status}, account={first_account}",
            )
            assert_equal(
                first_account,
                EXPECTED_ACCOUNT,
                f"首次打开登录后 Cookie 模拟站账号错误: actual={first_account}",
            )
            ...
            assert_equal(
                second_status,
                LOGGED_IN_STATUS,
                f"未删除本地缓存再次打开后 Cookie 模拟站状态错误: actual={second_status}, account={second_account}",
            )
            assert_equal(
                second_account,
                EXPECTED_ACCOUNT,
                f"未删除本地缓存再次打开后 Cookie 模拟站账号错误: actual={second_account}",
            )
            ...
            assert_equal(
                third_status,
                LOGGED_OUT_STATUS,
                f"删除本地缓存后 Cookie 模拟站应为未登录: actual={third_status}, account={third_account}",
            )
            ...
            try:
                if global_settings_snapshot is not None:
                    ...
                    global_settings_page.restore_global_settings_snapshot(
                        self._snapshot_with_cookie_enabled(global_settings_snapshot)
                    )
    ...
    def _cookie_credentials(self) -> tuple[str, str]:
        return local_auth_lab_login_credentials(self.config, "cookie")

    def _ensure_cookie_lab_user(self, username: str, password: str) -> None:
        settings = LocalAuthLabSettings.from_config(self.config).ensure_persistent_credentials()
        LocalAuthLabClient(settings).ensure_user("cookie", username, password)

    def _snapshot_with_cookie_enabled(self, snapshot: dict[str, object]) -> dict[str, object]:
        target = copy.deepcopy(snapshot)
        data_sync = target.get("data_sync")
        if isinstance(data_sync, dict):
            data_sync["cookie"] = True
        else:
            target["data_sync"] = {"cookie": True}
        return target
            ...
            with KernelCDPSession(
                ...
                if login_username:
                    local_auth_page.login(
                        login_username,
                        login_password,
                        run_id="global-settings-cookie-sync-disabled",
                    )
