...
CASE_MODULE = "全局设置"
ENVIRONMENT_NAME = "自动化-全局设置-不勾选Local Storage同步"
EXPECTED_ACCOUNT = "MCDL005"
LOGGED_IN_STATUS = "已登录"
LOGGED_OUT_STATUS = "未登录"


class TestDisableLocalStorageDataSync(unittest.TestCase):
    ...
    def test_local_storage_not_restored_after_cache_deletion_when_global_local_storage_sync_disabled(self) -> None:
        ...
        username, password = self._local_storage_credentials()
        ...
        try:
            ...
            assert_true(
                ...
                "数据设置 → 数据同步中的 Local Storage 未保持取消勾选状态",
            )
            self.logger.info(
                "Local Storage data sync is disabled: changed=%s",
    ...
    def _local_storage_credentials(self) -> tuple[str, str]:
        return local_auth_lab_login_credentials(self.config, "localstorage")
            ...
            with KernelCDPSession(
                ...
                if login_username:
                    local_auth_page.login(
                        login_username,
                        login_password,
                        run_id="global-settings-local-storage-sync-disabled",
                    )
