...
CASE_MODULE = "全局设置"
ENVIRONMENT_NAME = "自动化-全局设置-清除本地全部缓存-每次都清除-同步云端数据"
...
class TestGlobalSettingsClearAllCacheEveryOpenSyncCloud(unittest.TestCase):
    ...
    def test_clear_all_cache_every_open_then_sync_cloud_data_keeps_sites_logged_in(self) -> None:
        ...
        try:
            ...
            first_statuses = self._open_visit_sites_and_close(
                ...
                run_id_prefix="global-clear-all-cache-sync-cloud-first",
            )
            self._assert_sites_logged_in(first_statuses, stage="首次打开登录后")

            global_settings_page.open(force_reentry=True)
            global_settings_page.configure_clear_all_local_cache_every_open_sync_cloud_data()
            ...
            second_statuses = self._open_visit_sites_and_close(
                ...
                kernel_cdp_probe_timeout=kernel_cdp_probe_timeout,
                http_probe_timeout=http_probe_timeout,
                stage="启用全局清除本地缓存并同步云端后再次打开",
                wait_after_visit_seconds=2,
            )
            self._assert_sites_logged_in(second_statuses, stage="启用全局清除本地缓存并同步云端后再次打开")
        ...
        try:
            ...
            with KernelCDPSession(
                ...
                for site in SITE_DEFINITIONS:
                    ...
                    self.logger.info(
                        "Global clear-all-cache sync-cloud login status captured: "
