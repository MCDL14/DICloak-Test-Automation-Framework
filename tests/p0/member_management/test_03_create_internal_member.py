...
class TestCreateInternalMember(unittest.TestCase):
    @classmethod
    ...
    def test_create_internal_member_and_delete(self) -> None:
        ...
        try:
            ...
            assert_equal(
                member_page.edit_dialog_field_value("登录账号"),
                login_account,
                f"member login account did not match in edit dialog: {login_account}",
            )
            assert_true(
