...
- `python -m unittest discover -s tests/p1 -p "test_*.py"` 最近完整回归为 `Ran 230 tests ... OK`；P0 当前可发现 101 条，模块分布见 `README.md`；
- macOS、Linux 尚未记录实机验证结果，不以 Windows 结论替代两端实测；
- Cookie、Local Storage 与 IndexedDB 云端数据恢复业务用例均已由测试人员给出步骤并接入环境管理模块。原有六条已由逐用例 UI 快照恢复迁移到 `data_sync_config` 固定接口基准检查和按影响位恢复；2026-09-10 又从新环境 Cookie 持续保持用例拷贝新增一条接口生命周期变体，以接口创建/列表确认/删除，APP 仅负责搜索、打开、关闭和清空筛选，真实单跑 `1/1` 通过。
