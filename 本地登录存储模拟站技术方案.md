...
| 项目 | 内容 |
| --- | --- |
| 文档状态 | 框架、服务和页面已实现；Windows 实机验证通过；首条 Cookie 云端恢复业务用例已接入 |
| 适用项目 | DICloak 自动化框架 |
...
- `python -m unittest discover -s tests/p1 -p "test_*.py"` 当前共 99 条并全部通过，普通 `python run.py --precheck` 通过，P0 当前可发现 68 条；
- macOS、Linux 尚未记录实机验证结果，不以 Windows 结论替代两端实测；
- Cookie、Local Storage 与 IndexedDB 云端数据恢复业务用例均已由测试人员给出步骤并接入环境管理模块。本方案没有猜测未提供的云端接口或 APP DOM。
...
- P0 当前可发现 68 条；
- P1 在原 61 条基础上新增 Local Auth Lab、缓存安全、全局设置加载保护和数据同步精确勾选保护测试；持久凭据、旧 Session 自动迁移、认证状态专用包、远程执行顺序和无关 suite 仍强制同步认证状态的测试加入后曾达到 91 条；2026-08-04 新增创建环境抽屉默认分组、二次提交保护和列表 loading 刷新等待测试后，当前项目 P1 共 99 条并全部通过；
...
- Windows DICloak APP 中 Cookie、Local Storage、IndexedDB 三条正式数据校验用例已使用单一 Runner 串行通过，结果 `total=3 passed=3 failed=0 errors=0 skipped=0 flaky=0`；三站分别以 `MCDL001`、`MCDL002`、`MCDL003` 完成三次“已登录”读取及本地缓存删除后的云端恢复；
...
三个网站均支持注册、登录、当前账号与状态展示、主动退出和被动掉登；注册成功不自动登录，不创建 session，也不写入任何浏览器认证存储。系统提供固定 Origin、健康检查、生命周期管理、稳定 DOM 契约和受保护的自动化管理接口。Local Auth Lab 基础设施本身不内嵌业务步骤或断言；具体流程由外部用例组合。目前环境管理模块已接入 Cookie、Local Storage 和 IndexedDB 三条预置环境数据校验用例，以及对应的三条新环境持续保持用例。预置环境三条已完成 Windows 真实回归；新环境三条严格沿用原有默认创建流程，当前只完成静态验证和 P1 回归，默认创建问题解决前不记录真实通过结果。
...
- P1 完整回归为 `Ran 99 tests ... OK`；编译检查与 `git diff --check` 通过。
- “新环境 Cookie 持续保持”用例当前沿用原有 `create_environment()` 默认创建流程，不额外选择环境分组。此前显式选择 `未分组` 后曾完成 Windows 三阶段验证，但该处理已撤回；当前默认创建问题的解决方式待确认，不能将此前结果作为当前代码的通过结论。登录密码仍只保存在 Git 忽略的本地测试数据文件中。
- “新环境 Local Storage 持续保持”用例同样沿用原有 `create_environment()` 默认创建流程，不额外选择环境分组；使用本地测试数据中的 `MCDL005`，编排首次登录、关闭重开、3 秒等待、删除 19 位缓存目录后第三次恢复以及最终环境删除。当前只完成静态验证和 P1 回归，默认创建问题解决前不记录真实 APP 通过结果。
- “新环境 IndexedDB 持续保持”用例同样沿用原有 `create_environment()` 默认创建流程，不额外选择环境分组；使用本地测试数据中的 `MCDL006`，编排首次登录、关闭重开、3 秒等待、删除 19 位缓存目录后第三次恢复以及最终环境删除。当前只完成静态验证和 P1 回归，默认创建问题解决前不记录真实 APP 通过结果。
