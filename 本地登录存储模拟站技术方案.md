...
- `python -m unittest discover -s tests/p1 -p "test_*.py"` 最近完整回归为 `Ran 206 tests ... OK`；P0 当前可发现 100 条，模块分布见 `README.md`；
- macOS、Linux 尚未记录实机验证结果，不以 Windows 结论替代两端实测；
- Cookie、Local Storage 与 IndexedDB 云端数据恢复业务用例均已由测试人员给出步骤并接入环境管理模块。六条已由逐用例 UI 快照恢复迁移到 `data_sync_config` 固定接口基准检查和按影响位恢复；2026-08-25 联合真实回归 `total=6 passed=6 failed=0 errors=0 skipped=0 flaky=0`，用例内部合计 `509.67s`，比 2026-08-24 旧流程减少 `63.50s`。本方案没有猜测未提供的云端接口或 APP DOM。
...
三个网站均支持注册、登录、当前账号与状态展示、主动退出和被动掉登；注册成功不自动登录，不创建 session，也不写入任何浏览器认证存储。系统提供固定 Origin、健康检查、生命周期管理、稳定 DOM 契约和受保护的自动化管理接口。Local Auth Lab 基础设施本身不内嵌业务步骤或断言；具体流程由外部用例组合。目前环境管理模块已接入 Cookie、Local Storage 和 IndexedDB 三条预置环境数据校验用例，以及对应的三条新环境持续保持用例。六条在业务开始前通过组织配置 GET 检查固定数据同步基准，只有实际写入后才在结束阶段恢复当前数据同步影响位；普通流程不再逐条采集或恢复整页 UI 快照。2026-08-25 联合真实回归通过，结果 `total=6 passed=6 failed=0 errors=0 skipped=0 flaky=0`。
